...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  const config = await prisma.gitConfig.findUnique({ where: { projectId: params.id } });
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const config = await prisma.gitConfig.upsert({
    ...
    create: { repoUrl, branch: branch || "main", token, username, autoCommit: !!autoCommit, projectId: params.id, userId: (session.user as any).id },
  });
